...
class GRUCell(tf.contrib.rnn.RNNCell):
    # with tf r1.0 tf.nn.rnn_cell.RNNCell has been moved
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable(
                'W_r', shape=(self.state_size, self.state_size),
                initializer=tf.contrib.layers.xavier_initializer()
            )
            U_r = tf.get_variable(
                'U_r', shape=(self.input_size, self.state_size),
                initializer=tf.contrib.layers.xavier_initializer()
            )
            b_r = tf.get_variable(
            ...
            W_z = tf.get_variable(
                'W_z', shape=(self.state_size, self.state_size),
                initializer=tf.contrib.layers.xavier_initializer()
            )
            U_z = tf.get_variable(
                'U_z', shape=(self.input_size, self.state_size),
                initializer=tf.contrib.layers.xavier_initializer()
            )
            b_z = tf.get_variable(
            ...
            W_o = tf.get_variable(
                'W_o', shape=(self.state_size, self.state_size),
                initializer=tf.contrib.layers.xavier_initializer()
            )
            U_o = tf.get_variable(
                'U_o', shape=(self.input_size, self.state_size),
                initializer=tf.contrib.layers.xavier_initializer()
            )
            b_o = tf.get_variable(
